...
var ReviewPeriodCompleted = function () {
  ...
  _createClass(ReviewPeriodCompleted, [{
    ...
    value: async function create(web3, options, callback) {
      ...
      if (options.uuid === 'undefined') throw new Error('ReviewPeriodCompleted event requires UUID to follow.');

      var reviewPeriodEndBlock = await contract.getReviewPeriodEndBlock.call(options.uuid);

      if (reviewPeriodEndBlock.equals(0)) throw new Error('AuctionCompleted listener can only be activated once loan' + 'has been broadcasted');

      var reviewPeriodCompletedEvent = new ReviewPeriodCompleted(web3, reviewPeriodEndBlock);
