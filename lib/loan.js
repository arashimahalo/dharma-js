...
class Loan extends RedeemableERC20 {
  ...
  async refreshState() {
    ...
    return new Promise(async function(resolve, reject) {
      ...
      if (_this.state.equals(Constants.ACCEPTED_STATE)) {
        _this.interestRate = await _this.getInterestRate();
        const termBegin = await _this.events.termBegin();
        termBegin.get((err, logs) => {
          if (err) {
            reject(err)
          } else {
            const termBeginEvent = logs[0];
            _this.termBeginBlockNumber = termBeginEvent.blockNumber;
            _this.web3.eth.getBlock(_this.termBeginBlockNumber, (err, block) => {
              if (err) {
                reject(err)
              } else {
