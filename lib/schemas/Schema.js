class AddressType {
  constructor(web3) {
    this.web3 = web3;
  }

  validate(term) {
    if (!this.web3.isAddress(term))
      throw new Error('Address format is invalid');
  }
}

class Bytes32Type {
  validate(term) {
    if (!/0x[0-9A-Fa-f]{64}/g.test(term))
      throw new Error(term + ' is not a valid Bytes32Type');
  }
}

class Bytes1Type {
  validate(term) {
    if (!/0x[0-9A-Fa-f]{2}/g.test(term))
      throw new Error(term + ' is not a valid Bytes1Type');
  }
}

class NumberType {
  validate(term) {
    if (typeof term !== 'number')
      throw new Error(term + ' is not a valid number');
  }
}

class BooleanType {
  validate(term) {
    if (typeof term !== 'boolean')
      throw new Error(term + ' is not a valid boolean');
  }
}

class PeriodType {
  constructor(web3) {
    this.web3 = web3;
  }

  validate(term) {
    if (!(term === 'daily' ||
        term === 'weekly' ||
        term === 'monthly' ||
        term === 'yearly' ||
        term === 'fixed'))
      throw new Error('Invalid period type');
  }
}

class Schema {
  constructor(name, schema, optional=false) {
    this.name = name
    this.schema = schema;
    this.optional = false;
  }

  validate(terms) {
    if (!terms && !this.optional)
      throw new Error('Required parameter missing: ' + this.name);

    for (let key in this.schema) {
      if (!(key in terms)) {
        throw 'Required term ' + key + ' is missing.';
      } else {
        this.schema[key].validate(terms[key])
      }
    }
  }
}

module.exports = {Schema, BooleanType, Bytes32Type, Bytes1Type,
  AddressType, NumberType, PeriodType};
